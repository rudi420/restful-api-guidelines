...
[#208]
== icon:question-circle[1x] {MUST} define events compliant with overall API guidelines
...
[#194]
== icon:question-circle[1x] {MUST} treat events as part of the service interface
...
[#195]
== icon:question-circle[1x] {MUST} make event schema available for review
...
[#197]
== icon:question-circle[1x] {MUST} specify and register events as event types
...
[#213]
== icon:question-circle[1x] {MUST} follow naming convention for event type names
...
[#207]
== icon:question-circle[1x] {MUST} indicate ownership of event types
...
[#245]
== icon:question-circle[1x] {MUST} carefully define the compatibility mode
...
[#196]
== icon:question-circle[1x] {MUST} ensure event schema conforms to OpenAPI schema object
...
[#210]
== icon:question-circle[1x] {SHOULD} avoid `additionalProperties` in event type schemas
...
[#246]
== icon:question-circle[1x] {MUST} use semantic versioning of event type schemas
...
[#198]
== icon:question-circle[1x] {MUST} ensure that events conform to an event category
...
[#247]
== icon:question-circle[1x] {MUST} provide mandatory event metadata
...
[#211]
== icon:question-circle[1x] {MUST} provide unique event identifiers
...
[#201]
== icon:question-circle[1x] {MUST} use general events to signal steps in business processes
...
[#203]
== icon:question-circle[1x] {SHOULD} provide explicit event ordering for general events
...
[#202]
== icon:question-circle[1x] {MUST} use data change events to signal mutations
...
[#242]
== icon:question-circle[1x] {MUST} provide explicit event ordering for data change events
...
[#204]
== icon:question-circle[1x] {SHOULD} use the hash partition strategy for data change events
...
[#200]
== icon:question-circle[1x] {SHOULD} avoid writing sensitive data to events
...
[#214]
== icon:question-circle[1x] {MUST} be robust against duplicates when consuming events
...
[#212]
== icon:question-circle[1x] {SHOULD} design for idempotent out-of-order processing
...
[#199]
== icon:question-circle[1x] {MUST} ensure that events define useful business resources
...
[#205]
== icon:question-circle[1x] {SHOULD} ensure that data change events match the APIs resources
...
[#209]
== icon:question-circle[1x] {MUST} maintain backwards compatibility for events
