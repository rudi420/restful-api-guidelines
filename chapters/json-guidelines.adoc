...
[#167]
== icon:question-circle[1x] {MUST} use JSON as payload data interchange format
...
[#252]
== icon:question-circle[1x] {SHOULD} design single resource schema for reading and writing
...
[#250]
== icon:question-circle[1x] {SHOULD} be aware of services not fully supporting JSON/unicode
...
[#168]
== icon:question-circle[1x] {MAY} pass non-JSON media types using data specific standard formats
...
[#172]
== icon:question-circle[1x] {SHOULD} use standard media types
...
[#120]
== icon:question-circle[1x] {SHOULD} pluralize array names
...
[#118]
== icon:question-circle[1x] {MUST} property names must be snake_case (and never camelCase)
...
[#125]
[#240]
== icon:question-circle[1x] {SHOULD} declare enum values using UPPER_SNAKE_CASE string
...
[#235]
== icon:question-circle[1x] {SHOULD} name date/time properties with `_at` suffix
...
[#216]
== icon:question-circle[1x] {SHOULD} define maps using `additionalProperties`
...
[#123]
== icon:question-circle[1x] {MUST} use same semantics for `null` and absent properties
...
[#122]
== icon:question-circle[1x] {MUST} not use `null` for boolean properties
...
[#124]
== icon:question-circle[1x] {SHOULD} not use `null` for empty arrays

Empty array values can unambiguously be represented as the empty list, `[]`.


[#174]
== icon:question-circle[1x] {MUST} use common field names and semantics
...
[#249]
== icon:question-circle[1x] {MUST} use the common address fields
...
[#173]
== icon:question-circle[1x] {MUST} use the common money object
