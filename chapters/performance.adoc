...
[#155]
== icon:question-circle[1x] {SHOULD} reduce bandwidth needs and improve responsiveness
...
[#156]
== icon:question-circle[1x] {SHOULD} use `gzip` compression
...
[#157]
== icon:question-circle[1x] {SHOULD} support partial responses via filtering
...
[#158]
== icon:question-circle[1x] {SHOULD} allow optional embedding of sub-resources
...
[#227]
== icon:question-circle[1x] {MUST} document cacheable `GET`, `HEAD`, and `POST` endpoints
