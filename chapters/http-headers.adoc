...
[#133]
== icon:question-circle[1x] {MAY} use standard headers
...
[#132]
== icon:question-circle[1x] {SHOULD} use kebab-case with uppercase separate words for HTTP headers
...
[#178]
== icon:question-circle[1x] {MUST} use `Content-*` headers correctly
...
[#180]
== icon:question-circle[1x] {SHOULD} use `Location` header instead of `Content-Location` header
...
[#179]
== icon:question-circle[1x] {MAY} use `Content-Location` header
...
[#181]
== icon:question-circle[1x] {MAY} consider to support `Prefer` header to handle processing preferences
...
[#182]
== icon:question-circle[1x] {MAY} consider to support `ETag` together with `If-Match`/`If-None-Match` header
...
[#230]
== icon:question-circle[1x] {MAY} consider to support `Idempotency-Key` header
...
[#183]
== icon:question-circle[1x] {SHOULD} use only the specified proprietary Zalando headers
...
[#184]
== icon:question-circle[1x] {MUST} propagate proprietary headers
...
[#233]
== icon:question-circle[1x] {MUST} support {X-Flow-ID}
